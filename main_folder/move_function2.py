...
def cal_robotSpeed(robotSpeedX, robotSpeedX):
    robotSpeed = math.sqrt(robotSpeedX * robotSpeedX + robotSpeedY * robotSpeedY)
    return robotSpeed

def cal_robotDirectionAngle(robotSpeedX, robotSpeedX):
    robotDirectionAngle = math.atan2(robotSpeedY, robotSpeedX)
    return robotDirectionAngle

def cal_speed(robotDirectionAngle, wheelAngle, wheelDistanceFromCenter, robotAngularVelocity, wheelLinearVelocity):
...
def move_robot(state="translation", robotSpeedX, robotSpeedY, speed_limit=0.5, thrower_speed=0, failsafe=0):
